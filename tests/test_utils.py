...
def test_filter_vacancies_by_words_invalid_input() -> None:
...
def test_get_vacancies_by_salary_all_zero() -> None:
    vacancies = [
...
def test_get_top_vacancies_negative() -> None:
    with pytest.raises(ValueError):
        get_top_vacancies([], -1)


def test_get_top_vacancies_non_int() -> None:
    with pytest.raises(TypeError):
...
def test_vacancy_objects_for_json_normal() -> None:
    raw_data = [
        {
