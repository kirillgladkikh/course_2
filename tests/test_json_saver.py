...
#
#     def test_file_not_found(self, json_saver):
#         """Тест: файл не существует → возвращается шаблон с одной пустой вакансией."""
...
#
#     def test_empty_json_file(self, json_saver):
#         """Тест: файл существует, но пуст (например, []) → возвращается шаблон."""
...
#         assert vac.description == ""
#
#     def test_invalid_json_format(self, json_saver):
#         """Тест: файл содержит некорректный JSON → возвращается шаблон."""
...
#         assert vac.description == ""
#
#     def test_valid_json_single_vacancy(self, json_saver):
#         """Тест: файл содержит одну валидную вакансию → возвращается список из одного объекта Vacancy."""
#         sample_data = [
#             {
#                 "name": "Python Dev",
...
#
#
#     def test_valid_json_multiple_vacancies(self, json_saver):
#         """Тест: файл содержит несколько валидных вакансий → возвращается список объектов Vacancy."""
#         sample_data = [
#             {
#                 "name": "A",
#                 "salary": {"from": 50000},
...
#         assert result[1].salary_to == 80000
#         assert result[1].url == "url2"
#         assert result[1].description == "Desc2"
#
#
#     def test_json_with_extra_fields(self, json_saver):
#         """Тест: JSON содержит дополнительные поля → они сохраняются в объекте Vacancy."""
#         sample_data = [
#             {
#                 "name": "Test",
#                 "salary": {"from": 70000},
...
#
#     def test_salary_none_in_json(self, json_saver):
#         """Тест: salary: null в JSON → salary_from=0, salary_to=0."""
#         sample_data = [
#             {
#                 "name": "No Salary",
#                 "salary": None,
...
# === Тесты для write_vacancies::updated_vacancies ===
# # Вспомогательные данные
# def create_vacancy(name, salary, url, description, **kwargs):
#     """Создаёт экземпляр Vacancy с дополнительными атрибутами."""
#     vac = Vacancy(name, salary, url, description)
...
# @pytest.fixture
# def json_saver():
#     """Фикстура: экземпляр JSONSaver с mocked файловой системой."""
...
# @pytest.fixture
# def mock_file():
#     """Фикстура: mock для open(), чтобы не трогать реальные файлы."""
#     return mock_open()
#
#
# # --- Тестовые кейсы ---
# def test_write_vacancies_empty_storage(json_saver, mock_file):
#     """Тест: хранилище пустое → все новые вакансии добавляются."""
#     new_vacancies = [
#         {"name": "Dev1", "salary": {"from": 100}, "url": "url1", "description": "desc1"},
#         {"name": "Dev2", "salary": {"from": 200}, "url": "url2", "description": "desc2"},
#     ]
#
#     # Мокируем get_vacancies() → возвращает пустой список
#     with patch.object(json_saver, "get_vacancies", return_value=[]):
#         # Мокируем open()
#         with patch("builtins.open", mock_file):
#             json_saver.write_vacancies(new_vacancies)
#
#     # Проверяем, что в файл записаны именно новые вакансии
#     handle = mock_file()
#     written = json.loads(handle.write.call_args[0][0])
#
#     assert len(written) == 2
#     assert written[0]["name"] == "Dev1"
#     assert written[1]["name"] == "Dev2"
#     assert written[0]["url"] == "url1"
#     assert written[1]["url"] == "url2"
#
#
# def test_write_vacancies_no_duplicates(json_saver, mock_file):
#     """Тест: есть существующие вакансии → новые без дубликатов добавляются."""
#     existing = [
#         create_vacancy("DevA", {"from": 500}, "urlA", "descA", extra="x1"),
#         create_vacancy("DevB", {"from": 600}, "urlB", "descB", extra="x2"),
#     ]
#     new_vacancies = [
#         {"name": "Dev1", "salary": {"from": 100}, "url": "new_url1", "description": "desc1"},
#         {"name": "Dev2", "salary": {"from": 200}, "url": "new_url2", "description": "desc2"},
#     ]
#
#     with patch.object(json_saver, "get_vacancies", return_value=existing):
#         with patch("builtins.open", mock_file):
#             json_saver.write_vacancies(new_vacancies)
#
#     handle = mock_file()
#     written = json.loads(handle.write.call_args[0][0])
#
#     assert len(written) == 4  # 2 существующие + 2 новые
#     urls = [item["url"] for item in written]
#     assert "urlA" in urls
#     assert "urlB" in urls
#     assert "new_url1" in urls
#     assert "new_url2" in urls
#
#
# def test_write_vacancies_skip_duplicates(json_saver, mock_file):
#     """Тест: новые вакансии с URL, уже есть в хранилище → не добавляются."""
#     existing = [
#         create_vacancy("DevA", {"from": 500}, "url1", "descA"),
#     ]
#     new_vacancies = [
#         {"name": "Dev1", "salary": {"from": 100}, "url": "url1", "description": "new desc"},  # дубликат
#         {"name": "Dev2", "salary": {"from": 200}, "url": "url2", "description": "desc2"},  # новый
#     ]
#
#     with patch.object(json_saver, "get_vacancies", return_value=existing):
#         with patch("builtins.open", mock_file):
...
#     assert len(written) == 2
#     urls = [item["url"] for item in written]
#     assert urls.count("url1") == 1  # дубликат не добавлен
#     assert "url2" in urls
#
#
#
# def test_write_vacancies_filter_bad_urls(json_saver, mock_file):
#     """Тест: вакансии с url == 'Нет ссылки' или None не попадают в итоговый список."""
#     existing = [
#         create_vacancy("DevA", {"from": 500}, "urlA", "descA"),
#         create_vacancy("DevB", {"from": 600}, None, "descB"),          # должен быть исключён
#         create_vacancy("DevC", {"from": 700}, "Нет ссылки", "descC"),  # должен быть исключён
#     ]
#     new_vacancies = []  # новых нет
#
#     with patch.object(json_saver, "get_vacancies", return_value=existing):
#         with patch("builtins.open", mock_file):
#             json_saver.write_vacancies(new_vacancies)
...
#     assert written[0]["url"] == "urlA"
#
#
#
# def test_write_vacancies_preserve_extra_attrs(json_saver, mock_file):
#     """Тест: все дополнительные атрибуты Vacancy (кроме базовых) сохраняются в JSON."""
#     existing = [
#         create_vacancy(
#             name="Dev1",
#             salary={"from": 1000},
...
#
#     handle = mock_file()
#     written = json.loads(handle.write.call_args[0][0])[0]  # берём первый (и единственный) элемент
#
#     assert written["extra_field"] == "value1"
#     assert written["another"] == "value2"
#     # Базовые поля тоже на месте
#     assert written["name"] == "Dev1"
#     assert written["salary"] == {"from": 1000}
#     assert written["url"] == "url1"
#     assert written["description"] == "desc1"
#
#
#
# def test_write_vacancies_empty_new_list(json_saver, mock_file):
#     """Тест: новых вакансий нет → сохраняется только существующее."""
#     existing = [
#         create_vacancy("DevA", {"from": 500}, "urlA", "descA"),
#     ]
#     new_vacancies = []
